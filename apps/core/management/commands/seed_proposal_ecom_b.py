"""
Management command: seed_proposal_ecom_b

Окрема КП інтернет-магазину: Базовий 950 €, Преміум 1 500 €, Платінум 3 500 €.
Не чіпає shop-ecom-a7f3. Демо — стандартний магазин, не версія сайту клієнта.
"""
...
class Command(BaseCommand):
    help = 'Seed the second shop proposal (950/1500/3500) + classic demo-shop'
    ...
    def _provision_demo(self, proposal: Proposal) -> None:
        ...
        if shop is None:
            shop = DemoShop(
                proposal=proposal,
                name='Demo Shop',
                slug=DemoShop.generate_slug('ecom-b'),
            )
            shop.save()
