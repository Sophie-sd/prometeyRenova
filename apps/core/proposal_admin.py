...
def make_proposal_slug(client_name: str) -> str:
    base = slugify(client_name or 'kp')[:80] or 'kp'
...
class ProposalAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        slug_field.help_text = _(
            'Залиште порожнім — згенерується з імені клієнта + випадковий суфікс. '
            'URL: /proposal/<slug>/'
        )
    ...
    def clean(self):
        ...
        if slug:
            cleaned['slug'] = slug
            return cleaned
        candidate = make_proposal_slug(cleaned.get('client_name') or 'kp')
        qs = Proposal.objects.filter(slug=candidate)
        if self.instance.pk:
