"""Демо-сайти (корпоративні) в адмінці (агенція): без CRUD create — лише з
дії у `Proposal`."""
...
@admin.register(CorpSite)
class CorpSiteAdmin(UnfoldModelAdmin):
    list_filter_sheet = False
    list_display = ('name', 'slug', 'style_preset', 'has_catalog', 'is_active', 'created_at')
    list_filter = ('is_active', 'style_preset', 'has_catalog')
    ...
    def deactivate_action(self, request, queryset):
        ...
        self.message_user(request, _('Деактивовано %(count)d сайт(ів).') % {'count': queryset.count()})
    ...
    def activate_action(self, request, queryset):
        ...
        self.message_user(request, _('Активовано %(count)d сайт(ів).') % {'count': count})
